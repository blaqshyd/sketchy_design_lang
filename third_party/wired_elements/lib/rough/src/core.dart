...
class Line {
  ...
  bool intersects(Line line) {
    final o1 = getOrientation(source, target, line.source);
    final o2 = getOrientation(source, target, line.target);
    final o3 = getOrientation(line.source, line.target, source);
    final o4 = getOrientation(line.source, line.target, target);

    if (o1 != o2 && o3 != o4) {
      return true;
    }
    // source, target and line.source are colinear and line.source lies on
    // segment this.source-this.target
    ...
    // source, target and line.source are collinear and line.target lies on
    ...
    // line.source, line.target and source are collinear and source lies on
    // segment line.source-line.target
    if (o3 == PointsOrientation.collinear &&
        onSegmentPoints(line.source, source, line.target)) {
      return true;
    }

    // line.source, line.target and target are collinear and target lies on
    // segment line.source-line.target
  ...
  PointD? intersectionWith(Line line) {
    final yDiff = target.y - source.y;
    final xDiff = source.x - target.x;
    final diff = yDiff * (source.x) + xDiff * (source.y);
    final lineYDiff = line.target.y - line.source.y;
    final lineXDiff = line.source.x - line.target.x;
    final lineDiff = lineYDiff * (line.source.x) + lineXDiff * (line.source.y);
    final determinant = yDiff * lineXDiff - lineYDiff * xDiff;
  ...
  bool isMidPointInPolygon(List<PointD> polygon) => PointD(
    (source.x + target.x) / 2,
    (source.y + target.y) / 2,
  ).isInPolygon(polygon);
}
