...
class RoughDecorationPainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    final drawConfig = roughDecoration.drawConfig ?? DrawConfig.defaultValues;
    final filler = roughDecoration.filler ?? NoFiller();
    final generator = Generator(drawConfig, filler);
    final rect = offset & configuration.size!;

    final borderPaint = _buildDrawPaint(roughDecoration.borderStyle!, rect);

    final fillPaint = roughDecoration.fillStyle == null
        ? borderPaint
    ...
    switch (roughDecoration.shape) {
      ...
      case RoughBoxShape.circle:
        ...
        final diameter = configuration.size!.shortestSide;
